...
Inverse kinematics is, as the name suggests, the inverse problem of forward kinematics. Given a desired end-effector pose, we want to compute the joint parameters that achieve this pose. Unlike forward kinematics, inverse kinematics may not be analytically solvable for all robot structures, and may have multiple or no solutions.

Mathematically, forward kinematics allows us to compute the map
$$
x(q) = f(q)
$$
where $x$ is the end-effector pose and $q$ are the joint parameters. Inverse kinematics aims to find the inverse of $f$.
Given a desired end-effector pose $x_d$, we want to find $q$ such that
$$
x(q)- x_d = 0.
$$

<img src="IK_example_bijectivity.png" alt="IK_example_bijectivity" width="700"/>
